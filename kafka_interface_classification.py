...
# ŚCIEŻKA DO NOWEGO MODELU (upewnij się, że to folder który zapisałeś w treningu)
...
# (opcjonalnie) szybki debug: jakie są stage'e
...
# =========================
# 3) Predykcja (na całym DF, ale output maskujemy statusem)
# =========================
predicted = model.transform(flat)

# Jeśli model ma IndexToString, to będzie kolumna "predicted_class" (string).
# Jeśli nie ma, fallback: użyj numerycznego prediction jako string.
...
with_status = (
    ...
    .withColumn(
        "prediction_out",
        when(col("status") == "OK", prediction_text_col).otherwise(lit(None).cast("string"))
    )
    # (opcjonalnie) probability jako JSON string (przydatne do debug/pokazania pewności)
    .withColumn(
    "probability_out",
    when(col("status") == "OK", to_json(vector_to_array(col("probability"))))
